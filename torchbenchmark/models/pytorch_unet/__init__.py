...
class Model(BenchmarkModel):
    ...
    def __init__(self, test, device, batch_size=None, extra_args=[]):
        ...
        self.example_inputs = torch.rand(
            (self.batch_size, 3, 640, 959), dtype=torch.float32
        ).to(self.device)
        self.model = UNet(n_channels=3, n_classes=2, bilinear=True).to(self.device)
        if test == "train":
            self.sample_masks = torch.randint(
                0, 1, (self.batch_size, 640, 959), dtype=torch.int64
            ).to(self.device)
            self.model.train()
        elif test == "eval":
            self.model.eval()
        self.optimizer = optim.RMSprop(
            self.model.parameters(), lr=self.args.lr, weight_decay=1e-8, momentum=0.9
        )
    ...
    def jit_callback(self):
        if self.test == "eval":
            self.model = torch.jit.optimize_for_inference(
                torch.jit.freeze(
                    torch.jit.script(self.model.eval()), preserved_attrs=["n_classes"]
                )
            )
        else:
